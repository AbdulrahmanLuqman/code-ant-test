...
import { ArrowDown, Book, Cloud, Code, Home, LogOut, Settings, Support } from "./Icons"
    ...
    <aside className={`bg-white md:w-[242px] md:h-screen md:border border-x border-b flex flex-col gap-5 items-center px-5 py-6 md:sticky fixed transition-all duration-500 md:left-0 md:top-0 top-[62px] z-10 ${isNavOut ? "w-full left-0" : "left-[-900px]"}`}>
        ...
        <nav className="w-full h-full flex flex-col md:justify-between">
            <ul className="flex flex-col gap-2">
                {
                    NAVS.slice(0, 5).map(nav => <Link key={nav.name} to={nav.pathname}><li className={`w-fit md:w-full flex gap-3 items-center py-[10px] px-[14px] rounded-md cursor-pointer hover:bg-[whitesmoke] ${LOCATION.pathname === nav.pathname ? "md:bg-[#1570EF] md:text-white" : ""}`}>
                        <span>{nav.icon}</span>
            ...
            <ul className="flex flex-col gap-2">
                {
                    NAVS.slice(5, NAVS.length).map(nav => <Link key={nav.name} to={nav.pathname}><li className={`w-fit md:w-full flex gap-3 items-center py-[10px] px-[14px] rounded-md cursor-pointer hover:bg-[whitesmoke] ${LOCATION.pathname === nav.pathname ? "bg-[#1570EF] text-white" : ""}`}>
                        <span>{nav.icon}</span>
                        <span className="font-semibold">{nav.name}</span>
                    </li></Link>)
                }
